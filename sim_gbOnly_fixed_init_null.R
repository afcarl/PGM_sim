...
for (i in beg:end){
	...
	promoter_t <- matrix(ncol=res_gb,nrow=100)
	promoter_an <- matrix(ncol=res_gb,nrow=100)
	########################################################
	...
	for (current_sample in 1:length(ANs)) {
		...
		for (cpg in 1:11) {
			...
			cpg_list_pr[[cpg]] <- frequencies_gb
		}
	...
	###########################################################################################
	eval(parse(text=paste('write.table(x=t(c(pval_zscore,D,mean(Ds),sd(Ds))), col.names=FALSE, row.names=FALSE, append=TRUE, file="./',i,'.result")',sep="")))
